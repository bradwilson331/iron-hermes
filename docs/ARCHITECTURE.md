...
4. **AgentLoop runs.** `AgentLoop` sends the conversation history to an `AnyClient` (which wraps either `AnthropicClient` or an OpenAI-compatible endpoint via `reqwest`, with optional fallback provider). It streams `StreamEvent` chunks back, accumulating the assistant response.

5. **Tool calls are dispatched.** When the LLM response contains tool calls, `AgentLoop` dispatches each one through `ToolRegistry` in `ironhermes-tools`. Tools include `terminal`, `read_file` / `write_file` / `patch`, `web_search` / `web_read` / `web_extract`, a full browser control suite, `execute_code` (Python sandbox via `ironhermes-exec`), `delegate_task` (subagent spawning via `AgentSubagentRunner`), `memory_tool`, and any MCP-bridged tools registered by `McpManager`.
...
| `Tool` | trait | `crates/ironhermes-tools/src/registry.rs` | Single async `call()` method; every tool (terminal, file, web, browser, etc.) implements this |
...
│   │                           # compressor, subagent runner, memory manager. The core "brain".
...
├── providers/                  # Pluggable memory backend implementations.
