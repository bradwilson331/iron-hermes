...
---
*Last updated: 2026-05-11 — Phase 27.1.1 (Safe Foundation, hexapod TCP integration) complete. `hexapod_tcp` tool ships walk/stop/battery/distance/relax over TCP with a compile-time exhaustive allowlist; `on_session_end` Tool-trait hook fires a stop+relax safety halt on every CLI/TUI shutdown path; gap-01 collapsed 7 hand-rolled tool-registration sites into a canonical `register_defaults_except` entry point; gap-02 wired `set_toolset_config` at session start so `config.yaml`'s `tools.toolsets` section is enforced (closing the "Phase 20 wires real toolset state" TODO). 11/11 must-haves verified; manual UAT signed off on a powered robot.*
