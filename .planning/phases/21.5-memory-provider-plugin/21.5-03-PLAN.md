...
must_haves:
  truths:
    ...
    - "Grafeo on_pre_compress extracts entity-relationship triples from discarded messages and stores them in the graph"
  ...
  key_links:
    ...
    - from: "providers/memory-grafeo/src/lib.rs"
      to: "providers/memory-grafeo/src/schema.rs"
      via: "Entity and edge constants used during triple extraction"
      pattern: "EDGE_RELATES_TO"
---
...
// GrafeoDB API (all methods take &self — interior mutability built-in):
//   db.create_node(&[label]) -> NodeId
//   db.set_node_property(id, key, value)
//   db.create_edge(src, dst, edge_type) -> EdgeId
//   db.set_edge_property(id, key, value)
//   db.iter_nodes() -> impl Iterator<Item=NodeView>
//   db.delete_node(id) -> bool
//   db.create_property_index(prop_name)
// GrafeoDB is Send + Sync (verified: compiles inside MemoryProvider: Send + Sync).
```
...
CRITICAL: This method takes `&self` (NOT `&mut self`), because GrafeoDB uses interior
mutability — all its mutation methods (`create_node`, `set_node_property`, `create_edge`)
accept `&self`. This allows it to be called from `on_pre_compress` and `sync_turn`
which both take `&self` per the MemoryProvider trait.
...
fn find_or_create_entity(&self, name: &str) -> grafeo::NodeId {
...
**F2. Add `store_triples` helper** on `GrafeoMemoryProvider`:
...
fn store_triples(&self, triples: &[(String, String, String)]) {
    for (subject, relation, object) in triples {
        ...
        self.db.set_edge_property(edge_id, PROP_RELATION_TYPE, grafeo::Value::String(relation.clone().into()));
    }
}
```
...
Override `sync_turn` (D-07, D-12 — extract entities from memory entries and store as graph edges):
```rust
...
Override `on_pre_compress` (D-08, D-12 — extracts AND stores entity-relationship triples from discarded messages):
```rust
...
```
  ...
  <acceptance_criteria>
    ...
    - providers/memory-grafeo/src/lib.rs contains `fn store_triples` that calls `self.db.create_edge`
    - providers/memory-grafeo/src/lib.rs contains `fn find_or_create_entity(&self` (takes &self, NOT &mut self)
    - providers/memory-grafeo/src/lib.rs `on_pre_compress` calls `self.store_triples` (NOT just logging)
    - providers/memory-grafeo/src/lib.rs `sync_turn` calls `self.store_triples` (NOT just logging)
    ...
    - Test `test_store_triples_creates_entity_nodes_and_edges` passes
    - Test `test_on_pre_compress_stores_triples` passes
    - `cargo test -p memory-grafeo --features memory-grafeo` exits 0
  </acceptance_criteria>
  <done>Grafeo provider implements memory_recall (content substring match with relevance scoring), entity extraction helper, store_triples (creates Entity nodes and RELATES_TO edges in graph via GrafeoDB interior mutability), on_pre_compress and sync_turn both extract AND store triples (per D-08/D-12), system_prompt_block, queue_prefetch. All tests pass including test_on_pre_compress_stores_triples verifying actual graph mutation.</done>
</task>
...
| Compressed messages -> Grafeo entity graph | Message content from context compression extracted into entity-relationship triples |
...
| T-21.5-10 | Tampering | Grafeo on_pre_compress entity extraction | accept | Extracted triples are from messages that already passed scan_context_content. Entity names are stored as graph node properties (no injection vector). Heuristic extraction may produce false positives but these are benign (extra nodes, not security risk). |
</threat_model>
...
- `grep "store_triples" providers/memory-grafeo/src/lib.rs` shows calls in on_pre_compress AND sync_turn (not just logging)
- `grep "create_edge" providers/memory-grafeo/src/lib.rs` shows actual graph edge creation in store_triples
</verification>

<success_criteria>
Both Grafeo and DuckDB providers implement memory_recall with differentiated search strategies (graph content matching vs analytical ILIKE queries), all 4 advanced hooks (sync_turn, on_pre_compress, system_prompt_block, queue_prefetch) with provider-specific behavior, and full test coverage. Grafeo on_pre_compress and sync_turn extract AND store entity-relationship triples as graph edges (per D-08/D-12), not just log them. All tests pass.
</success_criteria>
