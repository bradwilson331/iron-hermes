...
Plans:
- [ ] 36.3.7.0-01-PLAN.md — Drop `--skills` argv from worker_spawn.rs; add HERMES_KANBAN_TASK_SKILLS env carrier; receiver-end test (Wave 1)
- [ ] 36.3.7.0-02-PLAN.md — Add cmd_kanban handler + dispatch arm + KanbanStoreReader trait + KanbanStoreReaderImpl + build_cmd_ctx wiring + receiver dispatch-chain test (Wave 1)
- [ ] 36.3.7.0-03-PLAN.md — D-12 determination commit + hook apply_circuit_breaker into detect_crashed_workers path + 2 receiver-end tests on the crashed path (Wave 1)
