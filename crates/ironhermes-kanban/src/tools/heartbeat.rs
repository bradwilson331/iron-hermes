        ...
        ToolSchema::new(
            ...
            json!({
                ...
                "properties": {
                    "task_id": {
                        "type": "string",
                        "description": "Task ID to heartbeat. Omit to use $IRONHERMES_KANBAN_TASK."
                    },
                    "note": {
                        "type": "string",
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<String> {
        ...
        let task_id = match task_id {
            ...
            None => {
                return Ok(crate::tools::common::reject_with_board(
                    "missing_task_id",
                    "task_id is required when $IRONIRONHERMES_KANBAN_TASK is not set",
                    Some(&board_ctx),
                ));
            }
        };

        // Resolve run_id: optional, from env via dual-read. `None` is fine — the column allows NULL.
        let run_id = crate::kanban_env("RUN_ID");
