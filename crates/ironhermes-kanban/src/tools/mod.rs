...
pub fn register_kanban_tools(
    ...
    registry.register(Box::new(KanbanShowTool::new(
        store.clone(),
        explicit_enable,
    )));
    registry.register(Box::new(KanbanListTool::new(
        store.clone(),
        explicit_enable,
    )));
    ...
    registry.register(Box::new(KanbanBlockTool::new(
        store.clone(),
        explicit_enable,
    )));
    registry.register(Box::new(KanbanCreateTool::new(
        store.clone(),
        explicit_enable,
    )));
    ...
    registry.register(Box::new(KanbanDecomposeTool::new(
        store.clone(),
        explicit_enable,
    )));
    // Phase 36.3.7.10 — kanban_specify (LLM-driven triage specifier, no fan-out).
    registry.register(Box::new(KanbanSpecifyTool::new(
        store.clone(),
        explicit_enable,
    )));
}
