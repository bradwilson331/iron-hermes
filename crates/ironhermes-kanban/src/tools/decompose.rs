//! `kanban_decompose` — LLM-driven triage decomposer (Phase 36.3.7.10).
//!
//! Fans a triage task out into a child-task swarm + promotes the parent
//! `triage → todo` via the [`crate::decomposer::decompose_triage_task`] kernel.
//!
//! # v1 Quiescence
//!
//! In v1 the LLM tool surface is **observable but quiescent**: the tool is
//! registered in the schema (orchestrators can see `kanban_decompose`) but its
//! `execute()` method returns a structured `no_aux_client` rejection envelope
//! directing the orchestrator to use the `hermes kanban decompose <id>` CLI
...
/// LLM tool: fan a triage task out into a child-task swarm (kanban_decompose).
...
impl Tool for KanbanDecomposeTool {
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<String> {
        ...
        // Orchestrators should use `hermes kanban decompose <id>` CLI verb instead.
        ...
        Ok(crate::tools::common::reject_with_board(
            "no_aux_client",
            "Decomposer not wired in this LLM tool runtime — use `hermes kanban decompose <id>` CLI verb instead",
            Some(&board_ctx),
        ))
    }
}
...
mod tests {
    ...
    #[test]
    fn kanban_decompose_name_and_toolset() {
        let store = make_store();
        let tool = KanbanDecomposeTool::new(store, true);
        assert_eq!(tool.name(), "kanban_decompose");
