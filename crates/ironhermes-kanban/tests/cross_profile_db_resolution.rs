...
//! `IRONHERMES_KANBAN_DB` for non-default board slugs (WARNING 6 bilateral pairing).
//!
//! Tests:
...
//!    `build_kanban_worker_env` emits `IRONHERMES_KANBAN_DB` into the child env Vec for a named board slug.
...
///
/// Each test that touches `IRONHERMES_KANBAN_DB` acquires this lock first and
/// removes the var on exit (via explicit cleanup or the guard Drop).
static ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// RAII guard that removes IRONHERMES_KANBAN_DB when dropped.
struct EnvGuard;
impl Drop for EnvGuard {
    fn drop(&mut self) {
        // Safety: test-only; ENV_LOCK serialises all env mutations in this file.
        unsafe { std::env::remove_var("IRONHERMES_KANBAN_DB") };
    }
}
...
// ---------------------------------------------------------------------------
// Test 1 (D-A1): open_from_env() reads IRONHERMES_KANBAN_DB and opens that path
// ---------------------------------------------------------------------------
...
fn open_from_env_reads_hermes_kanban_db() {
    ...
    unsafe { std::env::set_var("IRONHERMES_KANBAN_DB", db_path.to_str().unwrap()) };
    ...
    assert!(
        db_path.exists(),
        "DB file must be created at IRONHERMES_KANBAN_DB path"
    );
    drop(store);
}
...
fn open_from_env_fallback_when_unset() {
    ...
    // open_from_env() must not error when IRONHERMES_KANBAN_DB is absent.
...
fn open_from_env_err_on_unwritable_path() {
    ...
    // Safety: test-only; ENV_LOCK serializes env mutations.
    unsafe { std::env::set_var("IRONHERMES_KANBAN_DB", bad_path.to_str().unwrap()) };
...
// ---------------------------------------------------------------------------
// Test 4 (D-A2): open_from_env_or_board(Some("foo")) with IRONHERMES_KANBAN_DB set → env wins
// ---------------------------------------------------------------------------
...
fn open_from_env_or_board_env_wins_over_slug() {
    ...
    // Safety: test-only; ENV_LOCK serializes env mutations.
    unsafe { std::env::set_var("IRONHERMES_KANBAN_DB", env_db.to_str().unwrap()) };
    ...
    assert!(
        env_db.exists(),
        "IRONHERMES_KANBAN_DB path must be created when env wins"
    );
    drop(store);
}

// ---------------------------------------------------------------------------
// Test 5 (D-A2): open_from_env_or_board(None) with IRONHERMES_KANBAN_DB unset → kanban_db_path()
// ---------------------------------------------------------------------------
...
//   build_kanban_worker_env emits IRONHERMES_KANBAN_DB for a named board slug.
// ---------------------------------------------------------------------------
...
fn build_kanban_worker_env_emits_hermes_kanban_db() {
    ...
    let db_entry = env
        ...
        .expect("IRONHERMES_KANBAN_DB must be present in worker env for named board");

    // The value must contain the boards/my-board layout path.
    assert!(
        db_entry.1.contains("boards/my-board/kanban.db"),
        "IRONHERMES_KANBAN_DB must reference boards/my-board/kanban.db, got: {}",
        db_entry.1
    );
}
