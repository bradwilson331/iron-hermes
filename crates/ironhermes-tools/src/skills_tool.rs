...
const SKILLS_DESCRIPTION: &str =
    "Browse and activate skill documents. Actions: list, view, activate.";
...
fn handle_activate(registry: &SkillRegistry, args: &Value) -> Value {
    let name = match args.get("name").and_then(|v| v.as_str()) {
        Some(n) => n,
        None => {
            return json!({"status": "error", "message": "Missing required parameter 'name'"})
...
impl Tool for SkillsTool {
        ...
        ToolSchema::new(
            ...
            json!({
                ...
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": ["list", "view", "activate"],
                        "description": "Action to perform. list: show all skills; view: read full SKILL.md; activate: load skill body for use."
                    },
                    "name": {
                        "type": "string",
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<String> {
        ...
        let result = match action {
            ...
            other => {
                json!({"status": "error", "message": format!("Unknown action '{}'. Valid: list, view, activate", other)})
            }
        };
...
mod tests {
    ...
    fn make_tool_with_skills(skills: &[(&str, &str, &str)]) -> (SkillsTool, tempfile::TempDir) {
        ...
        let tool = SkillsTool::new(Arc::new(registry));
        (tool, dir)
    }
    ...
    async fn test_unknown_action_returns_error_with_valid_list() {
        ...
        assert!(msg.contains("list") && msg.contains("view") && msg.contains("activate"));
    }
