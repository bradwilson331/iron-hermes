...
fn validate_missing_api_key_returns_error() {
    ...
    assert!(
        ...
        "expected model.api_key error, got: {:?}",
        errors
    );
}
...
fn validate_fully_populated_returns_empty_vec() {
    ...
    assert!(
        errors.is_empty(),
        "fully populated config should produce no errors, got: {:?}",
        errors
    );
}
...
fn validate_yaml_roundtrip_stays_valid() {
    ...
    assert!(
        errors.is_empty(),
        "round-tripped config should still be valid, got: {:?}",
        errors
    );
}
...
fn validate_default_config_has_errors() {
    ...
    assert!(
        !errors.is_empty(),
        "Config::default() should have at least one validation error (missing api_key)"
    );
    assert!(
        errors.iter().any(|e| e.path == "model.api_key"),
        "should flag missing api_key"
    );
}
