...
fn config_set_preserves_other_keys() {
    ...
    assert_eq!(
        provider.as_deref(),
        Some("openrouter"),
        "model.provider should survive mutation of model.default"
    );
}
...
fn unknown_keys_survive_roundtrip_d15() {
    ...
    assert_eq!(
        got.as_deref(),
        Some("true"),
        "learning.skill_generation_enabled must survive — D-15 reservation"
    );
...
fn is_cache_breaking_uses_schema_correctly() {
    let s = schema();
    assert!(
        is_cache_breaking("model.default", &s),
        "model.default is cache_breaking per D-13"
    );
    assert!(
        !is_cache_breaking("memory.user_profile_enabled", &s),
        "memory.user_profile_enabled is NOT cache_breaking"
    );
}
...
fn config_set_returns_old_value_when_key_existed() {
    ...
    assert_eq!(
        second.as_deref(),
        Some("model-a"),
        "second set should return old value"
    );
}
...
fn unknown_top_level_section_survives_roundtrip() {
    ...
    assert_eq!(
        xyz_foo.as_deref(),
        Some("bar"),
        "xyz.foo must survive after setting model.default — Anti-Pattern #1 trap"
    );
