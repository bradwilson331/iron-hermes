...
// 49.4-02 fix: widened from `feature = "server"` to `not(target_arch =
// "wasm32")` — several pure helper fns below (profile_dir_for,
// read_env_keys, write_env_atomic_0600, render_profile_env_with_stamp,
// validate_key_name/value, save_profile_key_impl_with_stamp) were widened
// the same way (see `profile_dir_for`'s note) and need these types/traits
// in scope without requiring the `server` feature.
#[cfg(not(target_arch = "wasm32"))]
use secrecy::{ExposeSecret, SecretString};
#[cfg(not(target_arch = "wasm32"))]
use std::collections::HashMap;
// `HashSet` is only used by items that stayed `feature = "server"`-gated
// (not part of the 49.4-02 widened pure-fn cluster above).
#[cfg(feature = "server")]
...
/// `create_profile`).
// 49.4-02 fix (folded todo 2026-08-28): widened from `feature = "server"`
// to `not(target_arch = "wasm32")` — this fn is pure `PathBuf`/std::path
// (no `tokio/full` dependency), and many `server/*.rs` callers' own
// native-only helpers assume `not(target_arch = "wasm32")` is sufficient to
// reach it, which broke `cargo test --workspace` (no `server` feature
// requested for this non-default-member crate in that invocation). Wasm
// exclusion is unaffected — `not(target_arch = "wasm32")` excludes wasm
// exactly as `feature = "server"` did.
#[cfg(not(target_arch = "wasm32"))]
...
// 49.4-02 fix: widened from `feature = "server"` — see `profile_dir_for`'s
// note above (same rationale, pure std/dotenvy fn, no tokio dependency).
#[cfg(not(target_arch = "wasm32"))]
...
// 49.4-02 fix: widened from `feature = "server"` — see `profile_dir_for`'s
// note above; `save_profile_key_impl_with_stamp` (also widened) calls this.
#[cfg(not(target_arch = "wasm32"))]
...
// 49.4-02 fix: widened from `feature = "server"` — see `profile_dir_for`'s
// note above; `render_profile_env_with_stamp` (also widened) uses this.
#[cfg(not(target_arch = "wasm32"))]
...
// 49.4-02 fix: widened from `feature = "server"` — see `profile_dir_for`'s
// note above; pure string transform, no tokio dependency.
#[cfg(not(target_arch = "wasm32"))]
...
// 49.4-02 fix: widened from `feature = "server"` — see `profile_dir_for`'s
// note above; pure delegation to `ironhermes_core::dotenv_write`, no tokio.
#[cfg(not(target_arch = "wasm32"))]
...
// 49.4-02 fix: widened from `feature = "server"` — see `profile_dir_for`'s
// note above; pure string rendering, no tokio dependency.
#[cfg(not(target_arch = "wasm32"))]
...
// 49.4-02 fix: widened from `feature = "server"` — see `profile_dir_for`'s
// note above; pure std::fs atomic write, no tokio dependency.
#[cfg(not(target_arch = "wasm32"))]
...
// ============================================================================
// Phase 50.1 Plan 06 (D-18) / Phase 49.4 Plan 08 (D-18): archive_profile —
// a guarded MOVE into a sibling archive root, replacing the original
// permanent-removal implementation. Every pre-existing guard is preserved
// verbatim and in its original order; only the final step diverges (rename
// instead of `remove_dir_all`).
// ============================================================================

/// Phase 49.4 Plan 08 (D-18): the archive root's directory name — a SIBLING
/// of `PROFILES_SUBDIR` under `get_hermes_home()`, never nested inside it.
/// A sibling root is invisible to `list_profiles`'s directory scan by
/// construction (it only ever reads `get_hermes_home().join(PROFILES_SUBDIR)`)
/// — no new skip rule is needed there, which is exactly the must-have this
/// placement satisfies ("an archived profile disappears from the roster
/// without being destroyed").
#[cfg(feature = "server")]
pub(crate) const ARCHIVED_PROFILES_DIRNAME: &str = "archived-profiles";

/// Phase 50.1 Plan 06 (D-18, T-50.1-06-04): the one archive-protected
...
/// this archive-time guarantee changing with it. Checked at the impl layer
/// so the refusal holds even if a caller bypasses the UI entirely
/// (T-50.1-06-04). Name kept as `is_deletion_protected` — archiving is the
/// deletion-shaped operation this predicate has always protected against,
/// and `edit_dialog.rs`'s client-side mirror comment already documents this
/// name.
#[cfg(feature = "server")]
...
/// Phase 49.4 Plan 08 (D-18): current UTC timestamp formatted for a unique
/// archive directory suffix. Nanosecond precision so two archives issued in
/// immediate succession from the same test process still sort and (almost
/// always) differ; [`unique_archive_dir`] below is the actual collision
/// guarantee — this fn only supplies a human-legible starting point.
#[cfg(feature = "server")]
fn archive_timestamp_suffix() -> String {
    chrono::Utc::now().format("%Y%m%dT%H%M%S%.9fZ").to_string()
}

/// Phase 49.4 Plan 08 (D-18): atomically claim a not-yet-existing path under
/// `archive_root` for `name`, guaranteeing two archives of the same profile
/// name never collide — the timestamp suffix alone is not a hard guarantee
/// on every filesystem's clock resolution, so this loops appending a
/// numeric disambiguator until a candidate can be claimed.
///
/// WR-03: previously this only *checked* `Path::exists()` and left creating
/// anything to the caller's later `std::fs::rename` — a check-then-create
/// TOCTOU window where two near-simultaneous archives of the same profile
/// name could both observe the same candidate as free and then both rename
/// onto it (on POSIX, renaming a directory onto an existing *empty*
/// directory silently succeeds, so the second archive would clobber the
/// first). This now calls `std::fs::create_dir` on each candidate, which
/// atomically claims the slot (or fails with `AlreadyExists` if another
/// caller won the race), closing that window. The returned path exists as
/// an empty directory; the caller's `std::fs::rename` of the profile
/// directory onto it (or `copy_dir_recursive`'s cross-device fallback, whose
/// own `create_dir_all` is a no-op on an existing empty directory) then
/// populates it — both are POSIX-legal because it starts out empty.
#[cfg(feature = "server")]
fn unique_archive_dir(archive_root: &Path, name: &str) -> Result<PathBuf, String> {
    let base_suffix = archive_timestamp_suffix();
    let mut disambiguator: u32 = 0;
    loop {
        let candidate = if disambiguator == 0 {
            archive_root.join(format!("{name}-{base_suffix}"))
        } else {
            archive_root.join(format!("{name}-{base_suffix}-{disambiguator}"))
        };
        match std::fs::create_dir(&candidate) {
            ...
            Err(e) => return Err(format!("create_dir({candidate:?}): {e}")),
        }
    }
}

/// Phase 49.4 Plan 08 (D-18): whether a `std::fs::rename` error is the
/// cross-filesystem case (`EXDEV`) that `archive_profile_impl`'s fallback
/// branch exists to handle, as opposed to a permissions/not-found/other
/// error that should propagate unchanged.
#[cfg(feature = "server")]
fn is_cross_device_error(err: &std::io::Error) -> bool {
    err.kind() == std::io::ErrorKind::CrossesDevices
}

/// Phase 49.4 Plan 08 (D-18, supersedes Phase 50.1 Plan 06's permanent
/// `delete_profile_impl`): the `archive_profile` impl layer — a guarded MOVE
/// of an ordinary bot's profile directory into the sibling archive root.
/// Refuses the default profile ([`is_deletion_protected`]) and the
/// currently live profile (`ironhermes_core::current_profile()`) before
/// ever resolving a path, and confirms the resolved directory is contained
/// within the profiles root (canonicalized, following any symlink to its
/// real target) and is not itself a symlink before moving anything — never
/// follows a symlink out of the profiles root (T-49.4-08-02). Every guard
/// above is preserved VERBATIM and in the SAME ORDER as the permanent-delete
/// implementation it replaces (deliberately the actual prior code order —
/// default/live-profile checks on the raw name before validation — not a
/// reordering to "validate first": `is_deletion_protected` exists
/// specifically so "default" gets ITS OWN refusal message rather than
/// falling through to `validate_profile_name`'s generic "reserved name"
/// rejection, which validating first would make unreachable for that case).
///
/// Diverges from the prior implementation only at the final step: instead
/// of `std::fs::remove_dir_all`, the directory is renamed into
/// `ARCHIVED_PROFILES_DIRNAME` under a name carrying the original profile
/// name plus a UTC timestamp suffix ([`unique_archive_dir`]), so a second
/// archive of the same name never collides with (or overwrites) the first.
/// If the rename fails because the archive root lives on a different
/// filesystem (`EXDEV`), this falls back to a recursive copy
/// ([`copy_dir_recursive`], already used by `duplicate_profile_impl`)
/// followed by removing the source — the ONLY place a `remove_dir_all` call
/// may appear in this function, and it only runs after the copy has already
/// succeeded, so a partial copy can never destroy the sole copy of the
/// profile.
///
/// Bot-metadata cleanup (decision recorded, not silently applied): this
/// plan PRESERVES the prior behaviour of removing the profile's bot-meta
/// index entry on archive, unchanged. The archived directory itself still
/// carries the profile's own `config.yaml`/`SOUL.md` (the metadata a future
/// restore would need to re-seed a roster entry); the bot-meta index is a
/// separate UI-owned display-metadata cache
/// (`~/.ironhermes/bot-meta.json`) whose entry disappearing mirrors the
/// archived profile disappearing from the roster (this plan's own
/// must-have), and a restore path is future work this plan does not build.
///
/// Sibling avatar directory (a second decision recorded, changed from the
/// prior implementation): the permanent-delete predecessor best-effort
/// removed `bot_avatar_api::bot_avatar_dir(name)` (a directory sibling of
/// `PROFILES_SUBDIR`, never nested inside a profile dir). This plan
/// deliberately STOPS doing that — deleting a bot's uploaded/generated
/// avatar image is itself a destructive, non-reversible action, which
/// directly conflicts with D-18's whole point (archive, don't destroy). A
/// stray avatar directory left behind by an archive is an acceptable,
/// non-security-relevant leftover; nothing in this crate re-associates a
/// LATER-recreated profile of the same name with it without the operator
/// re-uploading/re-generating an avatar through the normal UI flow anyway.
#[cfg(feature = "server")]
pub(crate) fn archive_profile_impl(name: &str) -> Result<PathBuf, String> {
    if is_deletion_protected(name) {
        return Err("the default profile can't be archived".to_string());
    }
    if name == ironhermes_core::current_profile() {
        return Err(format!(
            "profile '{name}' is the currently live profile — it cannot be archived while it is serving the embedded runtime"
        ));
    }
    ...
    if profile_dir.is_symlink() {
        return Err(format!(
            "refusing to archive '{validated_name}': profile path is a symlink"
        ));
    }
    ...
    if !canonical_dir.starts_with(&canonical_root) {
        return Err(format!(
            "refusing to archive '{validated_name}': resolved path escapes the profiles root"
        ));
    }

    let archive_root = ironhermes_core::get_hermes_home().join(ARCHIVED_PROFILES_DIRNAME);
    std::fs::create_dir_all(&archive_root)
        .map_err(|e| format!("create_dir_all({archive_root:?}): {e}"))?;
    let archived_dir = unique_archive_dir(&archive_root, &validated_name)?;

    if let Err(rename_err) = std::fs::rename(&profile_dir, &archived_dir) {
        if !is_cross_device_error(&rename_err) {
            return Err(format!(
                "rename profile directory into archive: {rename_err}"
            ));
        }
        // Cross-filesystem fallback: copy first, remove the source only
        // after the copy has fully succeeded.
        copy_dir_recursive(&profile_dir, &archived_dir)
            .map_err(|e| format!("cross-filesystem archive copy failed: {e}"))?;
        std::fs::remove_dir_all(&profile_dir).map_err(|e| {
            format!("archive copy succeeded but removing the original directory failed: {e}")
        })?;
    }

    crate::server::bot_meta_api::delete_bot_meta_impl(&validated_name)?;

    // Phase 49.4 Plan 12 (D-16, UI-SPEC E14 error / operator decision): any
    // bot binding pointing at the just-archived profile reverts to the
    // default profile — never blocks or refuses the archive itself, which
    // has already succeeded by this point. Kept in the binding module
    // (single owner of binding state); this is only the one-line call from
    // "the archive path" the plan's own action text asks for.
    crate::server::bot_binding_api::revert_bindings_for_archived_profile(&validated_name);

    Ok(archived_dir)
}

/// Phase 49.4 Plan 08 (D-18): the `archive_profile` `#[server]` fn —
/// renamed from Phase 50.1 Plan 06's `delete_profile` (this doc comment
/// states plainly that this is a MOVE, not a removal). Callers updated:
/// `components/hermes_app/screens/bot_roster/delete_confirm.rs`'s
/// `DeleteBotConfirm` modal, whose destructive-confirmation body copy is
/// also corrected here to the UI-SPEC Copywriting Contract's locked
/// archive wording (it previously said "permanently deletes ... cannot be
/// undone", which the archive rewrite makes false). Follows this crate's
/// four-step write protocol: validate → fresh `Config::load()` →
/// `check_profile_write_gate` → `spawn_blocking` around
/// `archive_profile_impl`. There is no profile CLI subcommand to shell out
/// to in this workspace (RESEARCH.md Pitfall 3) — this is direct filesystem
/// move.
#[server]
pub async fn archive_profile(name: String) -> Result<(), ServerFnError> {
    #[cfg(feature = "server")]
    {
        let config = ironhermes_core::config::Config::load()
            .map_err(|e| ServerFnError::new(format!("Config load failed: {e}")))?;
        ...
        tokio::task::spawn_blocking(move || archive_profile_impl(&name))
            .await
            .map_err(|e| ServerFnError::new(format!("spawn_blocking join: {e}")))?
            .map_err(ServerFnError::new)?;
        Ok(())
    ...
    #[cfg(not(feature = "server"))]
    {
        let _ = name;
        Err(ServerFnError::new(
            "archive_profile unavailable without `server` feature",
        ))
    }
}
...
/// The reserved selector value for the ROOT/default profile — the master bot
/// (`$IRONHERMES_HOME` itself), distinct from any `profiles/<name>` entry.
/// Matches the Gateway scope selector's `"ROOT"` convention.
pub(crate) const ROOT_PERSONA_NAME: &str = "ROOT";

/// Phase 49.4: read the ROOT/default profile's persona — the master bot's
/// `$IRONHERMES_HOME/SOUL.md`, the base identity `PromptBuilder::load_soul_md`
/// actually loads. The root is NOT a `profiles/<name>` directory, so it has no
/// profile-name validation or existence check; an absent file is an empty body
/// (never an error), mirroring [`fetch_profile_persona_impl`]. Read-only: it
/// only `stat`s and (conditionally) reads, never creating anything on disk.
#[cfg(feature = "server")]
pub(crate) fn fetch_root_persona_impl() -> Result<ProfilePersona, String> {
    let soul_path = ironhermes_core::get_hermes_home().join("SOUL.md");
    let body = if soul_path.is_file() {
        std::fs::read_to_string(&soul_path).map_err(|e| format!("read root persona file: {e}"))?
    } else {
        String::new()
    };
    Ok(ProfilePersona {
        name: ROOT_PERSONA_NAME.to_string(),
        body,
    })
}

/// Phase 49.4: write the ROOT/default persona to `$IRONHERMES_HOME/SOUL.md`.
/// Same `validate_persona_body` + atomic-write protocol as
/// [`save_profile_persona_impl`], minus the profile-dir existence check — the
/// root always exists (it is the home directory itself), and the write gate is
/// enforced by the `#[server]` wrapper before this is reached.
#[cfg(feature = "server")]
pub(crate) fn save_root_persona_impl(body: &str) -> Result<(), String> {
    validate_persona_body(body)?;
    let soul_path = ironhermes_core::get_hermes_home().join("SOUL.md");
    write_persona_atomic(&soul_path, body)?;
    Ok(())
}
...
/// Phase 49.4: `#[server]` read of the ROOT/default persona (the master bot's
/// `$IRONHERMES_HOME/SOUL.md`). Read surface — no write gate, mirroring
/// `fetch_profile_persona`.
#[server]
pub async fn fetch_root_persona() -> Result<ProfilePersona, ServerFnError> {
    #[cfg(feature = "server")]
    {
        let persona = tokio::task::spawn_blocking(fetch_root_persona_impl)
            .await
            .map_err(|e| ServerFnError::new(format!("spawn_blocking join: {e}")))?
            .map_err(ServerFnError::new)?;
        Ok(persona)
    ...
    {
        Err(ServerFnError::new(
            "fetch_root_persona unavailable without `server` feature",
        ))
    }
}

/// Phase 49.4: `#[server]` write of the ROOT/default persona. Follows the same
/// four-step protocol as `save_profile_persona` (validate → fresh
/// `Config::load()` → `check_profile_write_gate` → `spawn_blocking`), writing
/// the master bot's `$IRONHERMES_HOME/SOUL.md`.
#[server]
pub async fn save_root_persona(body: String) -> Result<(), ServerFnError> {
    #[cfg(feature = "server")]
    {
        validate_persona_body(&body).map_err(ServerFnError::new)?;
        let config = ironhermes_core::config::Config::load()
            .map_err(|e| ServerFnError::new(format!("Config load failed: {e}")))?;
        check_profile_write_gate(&config).map_err(ServerFnError::new)?;
        tokio::task::spawn_blocking(move || save_root_persona_impl(&body))
            .await
            .map_err(|e| ServerFnError::new(format!("spawn_blocking join: {e}")))?
            .map_err(ServerFnError::new)?;
        Ok(())
    ...
    #[cfg(not(feature = "server"))]
    {
        let _ = body;
        Err(ServerFnError::new(
            "save_root_persona unavailable without `server` feature",
        ))
    }
}
...
// 49.4-02 fix: widened from `feature = "server"` — see `profile_dir_for`'s
// note above; pure char validation, no tokio dependency.
#[cfg(not(target_arch = "wasm32"))]
...
/// (T-48.2-07-04).
// 49.4-02 fix: widened from `feature = "server"` — see `profile_dir_for`'s
// note above; delegates only to other now-widened pure fns in this cluster
// (validate_key_name/validate_key_value/profile_dir_for/read_env_keys/
// render_profile_env_with_stamp/write_env_atomic_0600), no tokio dependency.
#[cfg(not(target_arch = "wasm32"))]
...
mod profile_scaffold_tests {
    ...
    // -------------------------------------------------------------------
    // Phase 49.4 Plan 08 (D-18, supersedes Phase 50.1 Plan 06's
    // delete_profile_impl): archive_profile_impl / is_deletion_protected.
    // -------------------------------------------------------------------

    /// Finds the single entry under `archive_root` whose name starts with
    /// `prefix` — the archive filename carries a timestamp suffix this
    /// helper does not need to know the exact value of.
    fn find_archived_dir(archive_root: &std::path::Path, prefix: &str) -> std::path::PathBuf {
        let matches: Vec<_> = fs::read_dir(archive_root)
        ...
        assert_eq!(
            matches.len(),
            1,
            "expected exactly one archived dir for prefix '{prefix}', found {matches:?}"
        );
        matches.into_iter().next().unwrap()
    }

    #[test]
    fn archive_profile_is_deletion_protected_returns_true_only_for_default() {
    ...
    #[test]
    fn archive_profile_impl_on_default_profile_returns_refusal_and_moves_nothing() {
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);
        ...
        let before = walk_all(dir.path());
        let result = archive_profile_impl("default");
        assert!(result.is_err());
        let after = walk_all(dir.path());
        assert_eq!(before, after, "the default profile refusal must move nothing");
    }

    #[test]
    fn archive_profile_impl_on_live_profile_returns_refusal_and_moves_nothing() {
        let dir = tempfile::tempdir().expect("tempdir");
        ...
        let result = archive_profile_impl("scout");
        assert!(result.is_err(), "the live profile must refuse archiving");
    }

    #[test]
    fn archive_profile_impl_on_ordinary_profile_moves_directory_and_removes_bot_meta_entry() {
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);
        ...
        let archived_path = archive_profile_impl("removable-bot").expect("archive should succeed");

        assert!(
            !target_dir.exists(),
            "the profile directory must no longer exist at its original path"
        );
        assert!(
            archived_path.is_dir(),
            "the returned archived path must exist"
        );
        assert!(archived_path.join("config.yaml").is_file());
        ...
        assert!(
            ...
            "the bot-meta index must no longer carry the archived profile's key"
        );
    }

    #[test]
    fn archive_profile_impl_leaves_sibling_bot_meta_entries_unchanged() {
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);
        ...
        archive_profile_impl("bot-a").expect("archive should succeed");
        ...
        assert!(
            ...
            "a sibling profile's bot-meta entry must survive unrelated to the archived one"
        );
        assert_eq!(
    ...
    #[test]
    fn archive_profile_impl_on_missing_profile_returns_not_found_error() {
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);

        let result = archive_profile_impl("never-existed");
        assert!(result.is_err(), "a nonexistent profile must not succeed silently");
    }

    #[test]
    fn archive_profile_impl_rejects_invalid_name_before_resolving_any_path() {
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);

        let before = walk_all(dir.path());
        let result = archive_profile_impl("../../etc/passwd");
        assert!(result.is_err());
        let after = walk_all(dir.path());
        assert_eq!(
            before, after,
    ...
    #[test]
    #[cfg(unix)]
    fn archive_profile_impl_refuses_symlinked_profile_directory() {
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);
        ...
        let result = archive_profile_impl("symlinked-bot");
    ...
    // -------------------------------------------------------------------
    // Phase 49.4 Plan 08 (D-18): the two NEW archive-specific behaviors —
    // byte-identical contents survive the move, and two archives of the
    // same profile name coexist as distinct directories.
    // -------------------------------------------------------------------

    #[test]
    fn archive_profile_impl_preserves_contents_byte_identical() {
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);

        let target_dir = profile_dir_for("preserved-bot");
        write_fixture_file(&target_dir.join("config.yaml"), "provider: openrouter\n");
        write_fixture_file(&target_dir.join(".env"), "OPENROUTER_API_KEY=sk-abc\n");
        write_fixture_file(&target_dir.join("SOUL.md"), "# Persona\nBe helpful.\n");
        write_fixture_file(&target_dir.join("logs").join("web.log"), "boot ok\n");
        write_fixture_file(&target_dir.join("state.db"), "not-really-sqlite-bytes");

        let archived_path = archive_profile_impl("preserved-bot").expect("archive should succeed");

        for (rel, expected) in [
            ("config.yaml", "provider: openrouter\n"),
            (".env", "OPENROUTER_API_KEY=sk-abc\n"),
            ("SOUL.md", "# Persona\nBe helpful.\n"),
            ("logs/web.log", "boot ok\n"),
            ("state.db", "not-really-sqlite-bytes"),
        ] {
            let actual = fs::read_to_string(archived_path.join(rel))
                .unwrap_or_else(|e| panic!("read archived {rel}: {e}"));
            assert_eq!(actual, expected, "archived '{rel}' must be byte-identical");
        }
    }

    #[test]
    fn archive_profile_impl_twice_produces_two_distinct_archived_directories() {
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);

        write_fixture_file(
            &profile_dir_for("twice-bot").join("config.yaml"),
            "first\n",
        );
        archive_profile_impl("twice-bot").expect("first archive should succeed");

        write_fixture_file(
            &profile_dir_for("twice-bot").join("config.yaml"),
            "second\n",
        );
        archive_profile_impl("twice-bot").expect("second archive should succeed");

        let archive_root = ironhermes_core::get_hermes_home().join(ARCHIVED_PROFILES_DIRNAME);
        let archived_entries: Vec<_> = fs::read_dir(&archive_root)
            .expect("read archive root")
            .filter_map(|e| e.ok())
            .map(|e| e.path())
            .filter(|p| {
        ...
        assert_eq!(
            ...
            2,
            "two archives of the same name must coexist as distinct directories, got {archived_entries:?}"
        );

        let contents: std::collections::HashSet<String> = archived_entries
            .iter()
            .map(|p| fs::read_to_string(p.join("config.yaml")).expect("read archived config"))
            .collect();
        assert_eq!(
            contents,
            std::collections::HashSet::from(["first\n".to_string(), "second\n".to_string()]),
            "the two archives must be independently readable, neither overwriting the other"
        );
    }

    #[test]
    fn unique_archive_dir_is_race_free_under_concurrent_claims() {
        // WR-03 regression test: the pre-fix implementation only checked
        // `Path::exists()` here and left creating anything to the caller's
        // later `std::fs::rename` -- a check-then-create TOCTOU window.
        // Exercise the atomic `create_dir`-based claim directly under real
        // concurrency: many threads racing to claim a slot for the SAME
        // profile name must all get distinct, freshly-created directories,
        // never the same one.
        let dir = tempfile::tempdir().expect("tempdir");
        let archive_root = dir.path().join("archive-root");
        std::fs::create_dir_all(&archive_root).expect("create archive root");

        const THREADS: usize = 16;
        let barrier = std::sync::Barrier::new(THREADS);
        let results: Vec<std::path::PathBuf> = std::thread::scope(|scope| {
            let handles: Vec<_> = (0..THREADS)
                .map(|_| {
                    ...
                    scope.spawn(move || {
                        barrier.wait();
                        unique_archive_dir(&archive_root, "race-bot").expect("claim a slot")
                    })
                })
                .collect();
        ...
        for path in &results {
            assert!(
                path.is_dir(),
                "every claimed slot must exist as a directory: {path:?}"
            );
            assert!(
                seen.insert(path.clone()),
                "two concurrent callers claimed the same archive slot: {path:?}"
            );
        }
        assert_eq!(seen.len(), THREADS, "every thread must claim a distinct slot");
    }

    #[test]
    fn archive_profile_impl_archive_root_is_sibling_of_profiles_root_not_nested_inside_it() {
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);

        write_fixture_file(
            &profile_dir_for("sibling-check-bot").join("config.yaml"),
            "x\n",
        );
        archive_profile_impl("sibling-check-bot").expect("archive should succeed");
        ...
        assert!(
            !archive_root.starts_with(&profiles_root),
            "the archive root must never be nested inside the profiles root"
        );
        assert_eq!(archive_root.parent(), profiles_root.parent());

        // The archived profile must be invisible to a scan of the profiles
        // root — the same scan shape `list_profiles` uses.
        let remaining: Vec<String> = fs::read_dir(&profiles_root)
            .expect("read profiles root")
            .filter_map(|e| e.ok())
            .map(|e| e.file_name().to_string_lossy().to_string())
            .collect();
        assert!(!remaining.contains(&"sibling-check-bot".to_string()));
        let _ = find_archived_dir(&archive_root, "sibling-check-bot-");
    }
}
...
mod profile_key_masking_tests {
    ...
    #[test]
    fn root_persona_save_and_fetch_round_trip_targets_home_soul_md() {
        // Phase 49.4: the ROOT/default persona is the master bot's
        // `$IRONHERMES_HOME/SOUL.md` — the exact path `PromptBuilder::load_soul_md`
        // reads. An absent file must read back as an empty body (never an
        // error), and a save must land at home root, not under any profiles/.
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);

        let empty = fetch_root_persona_impl().expect("fetch of absent root persona is Ok");
        assert_eq!(empty.name, ROOT_PERSONA_NAME);
        assert_eq!(empty.body, "", "absent root SOUL.md must read as empty body");

        save_root_persona_impl("Master identity body").expect("save root persona must succeed");

        assert_eq!(
            fs::read_to_string(ironhermes_core::get_hermes_home().join("SOUL.md"))
                .expect("root persona must land at $IRONHERMES_HOME/SOUL.md"),
            "Master identity body"
        );
        let fetched = fetch_root_persona_impl().expect("fetch after save is Ok");
        assert_eq!(fetched.name, ROOT_PERSONA_NAME);
        assert_eq!(fetched.body, "Master identity body");
    }

    #[test]
    fn save_profile_persona_round_trip_writes_canonical_path() {
        // Phase 49.4 Plan 01 (D-17) tracer test: pins the Soul screen's
        // save/load acceptance bar end to end — the write must land at
        // `canonical_persona_path`, never the legacy `workspace/SOUL.md`
        // fallback, and `fetch_profile_persona_impl` must read the exact
        // same bytes back through the real read path (not a stand-in).
        let dir = tempfile::tempdir().expect("tempdir");
        let _guard = home(&dir);
        std::fs::create_dir_all(profile_dir_for("alpha")).expect("seed profile dir");
        let body = "# Alpha\n\nYou are Alpha, a careful operator persona.";

        save_profile_persona_impl("alpha", body).expect("save must succeed");

        assert_eq!(
            fs::read_to_string(canonical_persona_path("alpha"))
                .expect("persona must land at the canonical profile-root path"),
            body,
            "save must write to canonical_persona_path, not a workspace fallback"
        );
        assert!(
            !profile_dir_for("alpha")
                .join("workspace")
                .join("SOUL.md")
                .exists(),
            "save must not also leave a copy at the legacy workspace/SOUL.md location"
        ...
        let fetched = fetch_profile_persona_impl("alpha").expect("fetch must succeed");
        assert_eq!(
            fetched.body, body,
            "fetch must read back the exact bytes written by save, byte-for-byte"
        );
    }
