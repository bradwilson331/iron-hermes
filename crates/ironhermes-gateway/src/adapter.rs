...
// Phase 36.7.1 Plan 01 Task 3: `MessageHandler` and `PlatformAdapter` moved
// to `ironhermes_core::adapter` so `ironhermes-restgw`'s `WebhookAdapter`
// can implement them without depending on `ironhermes-gateway` (which would
// otherwise cycle back through this crate's own dependency on
// `ironhermes-restgw` for `GatewayRunner::start()`'s construction site —
// see `ironhermes_core::adapter`'s own doc comment for the full rationale).
// Re-exported here UNCHANGED so every existing
// `use ironhermes_gateway::adapter::{MessageHandler, PlatformAdapter};`
// import in this codebase keeps resolving exactly as before.
pub use ironhermes_core::adapter::{MessageHandler, PlatformAdapter};
